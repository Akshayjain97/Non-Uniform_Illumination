...
if args.resume:
    path_to_file = '/sda/akshay/NUI/Results/inceptionv3/CIFAR10_B'+str(bs)+'_LR'+lr1+'_'+net1+'_'+optimizer1+'_intermediate.t7'
...
# Training
def train(epoch):
    f = open('/sda/akshay/NUI/Results/inceptionv3/CIFAR10_B'+str(bs)+'_LR'+lr1+'_'+net1+'_'+optimizer1+'.txt', 'a')
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
...
def test(epoch):
    ...
    if not os.path.isdir('CheckpointsResults'):
        os.mkdir('CheckpointsResults')
    torch.save(state, '/sda/akshay/NUI/Results/inceptionv3/CIFAR10_B'+str(bs)+'_LR'+lr1+'_'+net1+'_'+optimizer1+'_intermediate.t7')
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('CheckpointsResults'):
            os.mkdir('CheckpointsResults')
        torch.save(state, '/sda/akshay/NUI/Results/inceptionv3/CIFAR10_B'+str(bs)+'_LR'+lr1+'_'+net1+'_'+optimizer1+'.t7')
        best_acc = acc
